...
| 在庫（冷蔵庫）管理 | 食材名・数量（整数）・単位（個/g）を登録・編集・スワイプ削除。画像（冷蔵庫の写真・レシート・買い物メモ等）を **フォトライブラリから選ぶ／その場でカメラ撮影** すると Gemini が食材と数量を読み取り、在庫を自動登録。 |
| チラシ（特売品）管理 | 商品名・価格（任意）を登録・編集・スワイプ削除。画像（スーパーのチラシ・特売広告の写真等）を **フォトライブラリから選ぶ／その場でカメラ撮影** すると Gemini が商品名と価格を読み取り、特売品を自動登録。 |
| お買い物リスト | 決定済みの全献立の食材から、在庫（数量>0で名前一致）に足りないものだけを抽出して一覧表示。水・調味料（塩・砂糖・醤油・油など）は自動除外し、数量違いの同じ食材は1件にまとめる。チェックで消し込み、クリップボードへコピーして出力可能。 |
| ユーザー設定 | 家族人数（1〜10）、アレルギー、好みをトグルで設定。生成時に自動参照。 |
| 画面遷移・UI | 起動時スプラッシュ→フェード遷移、5タブ（献立／在庫／チラシ／買い物／設定）のボトムナビ、日本語UI。 |
...
- AI: Google Gemini `gemini-2.5-flash`（`google_generative_ai` パッケージ。献立生成に加え、画像からの在庫・チラシ読み取りにも使用）
- 画像取得: `image_picker`（フォトライブラリからの選択／カメラ撮影で在庫・チラシ画像を取得。iOS はカメラ利用に `NSCameraUsageDescription` を設定）
